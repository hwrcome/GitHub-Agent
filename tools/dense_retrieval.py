...
def hybrid_dense_retrieval(state, config):
    ...
    LOCAL_MODEL_PATH = "/data1/hanwenrui/model/colbert"
    cfg = config.get("configurable", {}) if isinstance(config, dict) else {}
    colbert_model_name = cfg.get("colbert_model_name", LOCAL_MODEL_PATH)
    alpha = cfg.get("retrieval_alpha", 0.7)
    logger.info(f"Loading ColBERT model from local path '{colbert_model_name}'...")
