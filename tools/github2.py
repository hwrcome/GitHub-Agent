    ...
    try:
        ...
        if row:
            ...
            if datetime.now() - updated_at < timedelta(days=expire_days):
                ...
                logger.info(f"🔄 缓存已过期: {repo_name}，准备重新拉取...")
    except Exception as e:
        logger.error(f"读取缓存失败 {repo_name}: {e}")
    return None
...
async def fetch_repo_documentation(repo_full_name: str, headers: dict, client: httpx.AsyncClient) -> str:
    # 🌟 1. 【拦截第一关】：先查本地缓存！
    cached_doc = get_from_cache(repo_full_name)
    if cached_doc:
        logger.info(f"⚡ 缓存命中秒出！直接使用本地数据: {repo_full_name}")
        return cached_doc

    logger.info(f"🌐 缓存未命中，开始拉取 GitHub API 并请求 LLM: {repo_full_name}")
    
    # ⬇️ -------- 以下为未命中缓存时，执行你原本的抓取逻辑 -------- ⬇️
    doc_text = ""
    ...
    # 将 AI 总结置于最开头，后面跟上清洗后的干净文本
    final_doc = f"{ai_enrichment}\n\n{cleaned_doc}"
    
    # 🌟 2. 【拦截最后一关】：把这次辛辛苦苦拉取和提纯的成果，存进数据库！
    save_to_cache(repo_full_name, final_doc)
    logger.info(f"💾 数据已存入本地 SQLite 缓存: {repo_full_name}")
